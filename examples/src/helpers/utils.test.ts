import { extractMetadata, parseMentionsMarkup } from './utils';

describe('helpers/utils', () => {
  test('extractMetadata', () => {
    const data = [
      [
        [],
        {
          metadata: {},
          mentionees: [{ type: 'user', userIds: [] }],
        },
      ],
      [
        [{ plainTextIndex: 3, id: 'valeriy', display: '@valeriy' }],
        {
          metadata: { mentioned: [{ index: 3, length: 7, userId: 'valeriy', type: 'user' }] },
          mentionees: [{ type: 'user', userIds: ['valeriy'] }],
        },
      ],
    ];

    data.forEach(([mentions, expected]) => {
      expect(extractMetadata(mentions as any)).toEqual(expected);
    });
  });

  test('parseMentionsMarkup()', () => {
    // index - position of @symbol
    // length - the length of the user's name, it does not include @symbol

    const data = [
      [
        '@valeriy',
        { mentioned: [{ index: 0, length: 7, type: 'user', userId: 'valeriyId' }] },
        '@[valeriy](valeriyId)',
      ],
      [
        'Hi @valeriy!',
        { mentioned: [{ index: 3, length: 7, type: 'user', userId: 'valeriyId' }] },
        'Hi @[valeriy](valeriyId)!',
      ],
      [
        'Edit mentions test @valeriy @valeriy-test @valeriy-test1',
        {
          mentioned: [
            {
              index: 19,
              length: 7,
              type: 'user',
              userId: 'valeriyId',
            },
            {
              index: 28,
              length: 12,
              type: 'user',
              userId: 'valeriy-test',
            },
            {
              index: 42,
              length: 13,
              type: 'user',
              userId: 'valeriy-test1',
            },
          ],
        },
        'Edit mentions test @[valeriy](valeriyId) @[valeriy-test](valeriy-test) @[valeriy-test1](valeriy-test1)',
      ],
      [
        '@Pang6 @pang2',
        {
          mentioned: [
            {
              index: 0,
              length: 5,
              type: 'user',
              userId: 'Pang6',
            },
          ],
        },
        '@[Pang6](Pang6) @pang2',
      ],
      [
        '@Pang6 @pang2 testtest   @pang2',
        {
          mentioned: [
            {
              index: 0,
              length: 5,
              type: 'user',
              userId: 'Pang6',
            },
            {
              index: 7,
              length: 14,
              type: 'user',
              userId: 'pang2 testtest',
            },
            {
              index: 25,
              length: 5,
              type: 'user',
              userId: 'pang2',
            },
          ],
        },
        '@[Pang6](Pang6) @[pang2 testtest](pang2 testtest)   @[pang2](pang2)',
      ],
    ];

    data.forEach(([text, metadata, expected]) => {
      expect(parseMentionsMarkup(text as any, metadata as any)).toEqual(expected);
    });
  });
});
